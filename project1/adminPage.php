...
if(isset($_GET['dishname'])){
    ...
    $query = 'SELECT d.DishID, f.Food_Name, v.Vendor_Name, d.Price,v.Image FROM dish d, vendor v, food f WHERE Food_Name = "'. $dishname .'" AND d.FoodID = f.FoodID AND v.VendorID = d.VendorID';
    mysqli_select_db($conn,"user");
    $result = mysqli_query($conn,$query);
    $count = mysqli_num_rows($result);
...
if(isset($_GET['dishwhere'])){
    ...
    $query = 'SELECT d.DishID, f.Food_Name, v.Vendor_Name, d.Price,f.Image FROM dish d, vendor v, food f WHERE Vendor_Name = "'. $dishwhere .'" AND d.FoodID = f.FoodID AND v.VendorID = d.VendorID';
    mysqli_select_db($conn,"user");
    $result = mysqli_query($conn,$query);
    $count = mysqli_num_rows($result);
...
if (isset($_GET['emailsearch']) && isset($_GET['usernamesearch']) && isset($_GET['phonesearch']) ){
    ...
    $query = 'SELECT UserID, Email, Password, Phone, Address, First_Name, Last_Name, Birthday FROM account WHERE Email LIKE "%'. $email .'%" AND Phone LIKE "%'. $phone .'%" AND (First_Name LIKE "%'. $name .'%" OR Last_Name LIKE "%'. $name .'%" )';
    mysqli_select_db($conn,"user");
    $result = mysqli_query($conn,$query);
    $count = mysqli_num_rows($result);
...
if(isset($_GET['accountemail'])){
    $name = mysqli_real_escpe_string($conn, $_GET['accountemail']);
    $query = 'SELECT t.TransID, f.Food_Name, t.Quantity, t.Total, t.Date FROM transaction t, account a, food f, dish d WHERE t.UserID = a.UserID  AND a.Email = "'.$name.'" AND d.DishID = t.DishID AND f.FoodID = d.FoodID';
    mysqli_select_db($conn,"user");
    $result = mysqli_query($conn,$query);
    $count = mysqli_num_rows($result);
