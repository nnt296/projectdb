...
function fetchAccountDetail(response) {
    var array = JSON.parse(response);
    var text = "";
    document.getElementById("accountresult").innerHTML = "<tr><button onclick=accountSearch()>Back</button></tr><tr><th>TransID</th><th>Food</th><th>Quantity</th><th>Total</th><th>Date</th></tr>";
    for (i = 0; i < array.length; i++) {
            text += transactiontDisplay(array[i]['TransID'],
                                array[i]['Food_Name'],
                                array[i]['Quantity'],
                                array[i]['Total'],
                                array[i]['Date'],
...
function transactionDisplay(id, foodname, quantity, total, date) {
    var text = '<tr>';
    text += '<td>' + id + '</td>';
    text += '<td>' + foodname + '</td>';
    text += '<td>' + quantity + '</td>';
    ...
    if(choice === true){
        ...
        deleteAccount.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
                if(this.responseText === "true"){
                    alert('Delete Successfully');
                    clickAccount(str.substr(37,str.length-37));
                }
                if(this.responseText === "false"){
                    alert('Delete Unsuccessfully');
                }
    ...
    if(choice === true){
        ...
        deleteDish.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
                if(this.responseText === "true"){
                    alert('Delete Successfully');
                    if(mode === 1)clickVendor(str.substr(37,str.length-37));
                    if(mode === 2)clickFood(str.substr(37,str.length-37));
                }
                if(this.responseText === "false"){
                    alert('Delete Unsuccessfully');
                }
